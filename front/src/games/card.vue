<template>
  <div @click="$emit('click', card)"
    :class="{absolute:1, bgYellow:1, circle15px:1, 'elevation-24':1, fadeOut:card.fadeOut}"
    :style="{
    ...
    <div v-if="card.fxRed" class="bgRed circle absolute fadeOut" :style="{opacity:0, left:'-100%', top:'-71.3%', width: (card.position.width * 3) + 'px', height: (card.position.width * 3) + 'px'}">
