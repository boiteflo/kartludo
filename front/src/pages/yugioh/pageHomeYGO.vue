<template>
  <header-yugioh>
    <div class="pageTop">
      ...
      <br>
      <div class="flex-wrap">
        <div v-for="group in getLimitFriendsGroups(store.cards)" v-bind:key="group.id"
          style="position:relative; border: 5px solid red; margin: 5px 5px 5px 20px">
          <div style="position:absolute; top:-15px; left:-15px;">
            <div class="s25"
              style="color:red; text-align:center; font-style:bold; border-radius:15px; background:black; outline: 5px solid red">
              1
            </div>
          </div>
      ...
      <panel-cards :cards="store.cards.filter(x => x.Bonus && x.Limit !== '0')" :badgeoff="false" :center="true"
        tooltip="text">
      </panel-cards>
      <v-card class="bg2 colorWhite" style="margin-top:20px;">
        <v-card-title>Easter Eggs</v-card-title>
        <v-card-text class="bg2 colorWhite">Il y a actuellement 5 Easter-Eggs visuels a trouver sur le site dont 4
          liés
          a des cartes.</v-card-text>
      </v-card>

      <v-card class="bg">
        <v-card-title>Remerciement</v-card-title>
        <v-card-text class="bg">Ce format a été inventé par Zouloux (07/2023). Le Site Web a été créé par
          FlorentOutan.
          (Contactez moi sur discord). Avec la participation de Lockai, Aelle, Tenebros, Bludzee, Yan_Ue4. Utilisation
          de la base de données : https://db.ygoprodeck.com.</v-card-text>
      </v-card>
    </div>
  </header-yugioh>
</template>
...
export default {
  name: 'pageHomeYGO',
  components: { headerYugioh, panelCards },
  data: () => ({
    store: store,
  }),
  methods: {
