    ...
    <div v-if="true">
      ...
      <div class="flex flex-wrap m5px">
        ...
        <div v-for="card in cards.Ancestry" :key="'Ancestry' + card.name">
          <div ref="card" class="container">
            <img class="overlay-image" :src="require('@/assets/Images/ancestry/' + card.id + '.jpg')" />
            <div class="background-image cardAncestry">
              <div class="text text-bold-left" :style="{ top: '240px', 'font-size': '30px' }">{{ card.name }}
              </div>
              <div class="text text-normal" :style="{ top: '285px' }" v-html="card.description"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-wrap m5px">
        ...
        <div v-for="community in cards.Community" :key="'Community' + community.name">
          <div ref="card" class="container">
            <img class="overlay-image" :src="require('@/assets/Images/community/' + community.id + '.jpg')" />
            <div class="background-image cardCommunity">
              <div class="text text-bold" :style="{ top: '245px', 'font-size': '20px' }">{{ community.name }}</div>
              <div class="text text-normal" :style="{ top: '280px' }" v-html="community.description"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-wrap m5px">
        ...
        <div v-for="cardClass in cards.Class" :key="'Class' + cardClass.name">
          <div ref="card" class="container">
            <img class="overlay-image" :src="require('@/assets/Images/class/' + cardClass.id + '.jpg')"
              style="top:-20px" />
            ...
            <img class="ruban" :src="require('@/assets/Images/class/' + cardClass.id + '-banner.webp')" />
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-wrap m5px">
        ...
        <div v-for="cardClass in cards.Class" :key="'Class' + cardClass.name" style="display:inline">
          <div v-for="cardSousClass in cardClass.SousClass" :key="'SousClass' + cardSousClass.name"  style="display:inline">
            <div ref="card" class="container">
              <img class="overlay-image" :src="require('@/assets/Images/class/' + cardClass.id + '.jpg')" />
      ...
      <div class="flex flex-wrap m5px">
        ...
        <div v-for="card in cards.Domain" :key="'Domain' + card.name">
          <div ref="card" class="container">
            <img class="overlay-image" :src="require('@/assets/Images/domain/' + card.domaine.toLowerCase() + '.jpg')"
              style="top:-8px" />
            <div class="background-image cardDomain">
              ...
              <div class="text text-normal" :style="{ top: '280px', 'font-size': '11px' }" v-html="card.description">
              </div>
            </div>
            <img class="ruban"
              :src="require('@/assets/Images/domain/banner-' + card.domaine.toLowerCase() + '.webp')" />
            <img class="cost" :src="require('@/assets/Images/other/cost.webp')" />
            <div class="bgDomain" :style="{ 'background-color': getDomainColor(card.domaine) }"></div>
            <div class="text"
              :style="{ top: '15px', left: '45px', 'font-size': '30px', color: card.domaine === 'Bone' ? 'black' : 'white', 'z-index': 2, 'font-weight': 'bold' }">
              {{ card.niveau }}</div>
            <div class="text"
              :style="{ top: '24px', right: '44px', 'font-size': '25px', color: 'white', 'z-index': 2 }">
              {{ card.cost }}
            </div>
          </div>
        </div>
...
.cardAncestry {
  background: url('@/assets/Images/template/CardAncestry.png');
}

.cardCommunity {
  background: url('@/assets/Images/template/CardCommunity.png');
}

.cardClass {
  background: url('@/assets/Images/template/CardClass.png');
}

.cardDomain {
  background: url('@/assets/Images/template/CardDomain.png');
}

.cardSousClass {
  background: url('@/assets/Images/template/CardSousClass.png');
}
</style>
...
data: () => ({
  ...
  capturedImage : null,
  showOrigin: true,
  elf : "<p><i>Les Elfes sont des grand humanoides avec les oreilles pointues et des sens aigisés.</i><br><strong>Transe spirituelle :</strong> Pendant un repos, vous pouvez entrer en transe et effectuer une action suppl&eacute;mentaire de temps d'arr&ecirc;t.<br><strong>R&eacute;actions Rapides :</strong> Vous pouvez marquer un Stress pour b&eacute;n&eacute;ficier d'un avantage sur un Jet de R&eacute;action.</p>"
}),
async mounted(){
...
methods: {
  ...
  async getImages(){
    await this.$refs.card.forEach(div => this.getImage(div));
  ...
  drawCanvas(canvas, classObj) {
    ...
    const imagePromises = [
      require('@/assets/Images/template/CardClass.png'),
      require('@/assets/Images/class/' + classObj.id + '.jpg')
    ...
    image.src = require('@/assets/Images/template/CardAncestry.png');
    image.onload = () => {
