<template>
  <div>
    <!--<text-quill :text="tex" @change="tex= $event"></text-quill>
  {{ tex }}-->
      ...
      <div class="flex flex-responsive">
        <div class="w100p">
          ...
          <div v-for="index in texts.length" :key="'TextAdd' + index">
            <text-quill class="w100p" :text="texts[index - 1]" @change="setText(index, $event)"></text-quill>
          </div>
          <br>
          <br>
...
.text p {margin-bottom:0px !important;}
</style>
...
<script>
  ...
  export default {
  name: 'pageTemplate',
  components: {hierarchy, buttonBig, textQuill},
  data: () => ({
    choice:0,
    templateImages: null,
    refreshCard:0,
    tex: "<p>test <em>it </em><strong>gras</strong></p>",
